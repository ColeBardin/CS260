...
char identical(BST *S1, BST *S2) {
    /* Error handle improper usage of function */
    if(S1 == NULL || S1 == NULL){
        return 0;
    }
    /* Begin recursive checks at root of each tree */
    return isSameTree(S1->root, S2->root);
}

char isSameTree(Node *N1, Node *N2) {
    /* If both Nodes are NULL, successfully reached end of one branch */
    if(N1 == NULL && N2 == NULL){
        return 1;
    }
    ...
    /* If nodes have different values, trees are not identical */
    if(N1->value != N2->value){
        return 0;
    }
    /* If they have identical values, recursively check child trees. Make sure both sides are identical */
