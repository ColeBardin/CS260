/**
    ...
    This file uses a Heap to implement Huffman Encoding and provides other heap tools
 */
...
void huffman(char *filename) {
  ...
  int huffCode[128]; /* Individual character huffman codes */
  ...
  getHuffmanCodes(huffCode, huffmanHeap->data[0]->root, 0);
...
void getHuffmanCodes(int *A, Node *current, int code) {
  ...
  if (current->name == -1) {
    ...
    getHuffmanCodes(A, current->left, code << 1);
    getHuffmanCodes(A, current->right, (code << 1) | 1);
  } else {
...
void downHeap(Heap *myHeap, int i) {
  ...
  if (rightIndex >= myHeap->currentSize) {
    ...
    minIndex = myHeap->data[leftIndex]->root->value >
                       myHeap->data[rightIndex]->root->value
                   ? rightIndex
                   : leftIndex;
  }
...
void heapSwap(Heap *myHeap, int i, int j) {
  ...
  /* If there currently isn't 2 elements or heap cannot store 2 elements, cannot
   * swap */
...
void printHeap(Heap *myHeap) {
  ...
  printf("Contents:\n");
  for (int i = 0; i < myHeap->currentSize; i++) {
    printf("root: %d(%c) - %.5f\n", myHeap->data[i]->root->name,
           myHeap->data[i]->root->name, myHeap->data[i]->root->value);
  }
  printf("\n");
} /* Thanks! */

void printBin(int num) {
  bool print = false; /* Used to not print leading zeros */
