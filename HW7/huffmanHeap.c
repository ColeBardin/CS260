...
void huffman(char *filename){
    ...
    int i; /* Used for various indexing in this scope */
    ...
    int compressedFileSize = 0; /* Stores calculated size of compressed file */
    bool debug = false; /* Set to true to calculate compressed file size */
    ...
    for(i = 0; i < 128; i++){
        if(charCount[i] > 0){
            /* Allocate memory for huffman code string */
            huffCode[i] = malloc(sizeof(char) * 30);
    ...
    /* Output calculated compressed file size if debug is specified above */
    if(debug == true){
        /* Calculate compressed file size */
        for(i = 0; i < 128; i++){
            if(charCount[i] > 0){\
                compressedFileSize += charCount[i] * strlen(huffCode[i]);
            }
        }
        printf("\n%s\n", filename);
    ...
    for(i = 0; i < 128; i++){
        if(charCount[i] > 0){
            /* Predecrement pointer since it was incremented earlier */
            free(--huffCode[i]);
        }
    }
